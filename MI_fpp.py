...
class fpp:
    def __init__(self, sparse=False, randomSeed = 1234, printOutput = False, debugMode = False):
    ...
    def setupPointCloud(self, sample, batchSize=128, lr=1e-3, mi_type='mine'):
        self.sample = sample
        ...
        self.x = tf.placeholder(tf.float32, shape=[None, x_dim], name="input_sample")
        ...
        # x = tf.placeholder(shape=[None, x_dim], tf.float32)
        # z = tf.placeholder(shape=[None, 2], tf.float32)
        self.proj_mi_estimator = Mi_estimator(regu_name = mi_type,
                                       dim_x = [x_dim],
                                       dim_z = [2],
                                       ...
                                       ema_decay=0.99,
                                       negative_samples=1)
        self.estimator_train_op, self.estimator_quantities = self.proj_mi_estimator(self.x, self.x_hat)

        self.reconstruction_loss = self.estimator_quantities['mi_for_grads']
    ...
    def trainPointCloud(self, epoches=50, minimalStep = 100):
        self.sampleBuffer = self.sample.copy()
        # self.fBuffer = self.f.copy()

        step = int( float(len(self.sampleBuffer)) / float(self.batchSize) * epoches)
        ...
        # if self.sample.shape[0]<self.batchSize:
        #     self.batchSize = self.sample.shape[0]//2
        ...
        for i in range(step):
                ...
                start_idx = (i * self.batchSize) % len(self.sampleBuffer)
                if start_idx + self.batchSize >= len(self.sampleBuffer):
                    ...
                    # print("restart")
                batch = self.sampleBuffer[ start_idx:start_idx+self.batchSize, :]
                # print(batch.shape, batch.dtype)
                if i==0:
                    self.sess.run(tf.global_variables_initializer(), feed_dict={self.x:batch})
                # y_batch = self.fBuffer[start_idx:start_idx+self.batchSize, :]
                # Then update estimator
                # sess.run(self.estimator_train_op, feed_dict={x: ..., z= ...})
                # _, self.loss, self.projMat = self.sess.run([self.optimizer, self.reconstruction_loss, self.U],feed_dict={self.x:batch,self.y:y_batch})
                _, self.MI, self.projMat = self.sess.run([self.optimizer, self.estimator_quantities["mi_for_grads"], self.U],feed_dict={self.x:batch})
                self.sess.run(self.estimator_train_op, feed_dict={self.x:batch})
                if self.printOutput:
                    if i%50==0:
                        print('step {:d} mutual information estimation: {:.2f}'.format(i, self.MI))
                    if i==step-1:
                        print('step {:d} mutual information estimation: {:.2f}'.format(i, self.MI))
    ...
    def setup(self, sample, f, regressorType="polynominal", degree=4, lr=1e-3, reg_weight=1e-4,
            MI_regu=False, MI_weight=1e-3, MI_batchSize=128, MI_type='mine'):

        #### MI related #####
        self.MI_weight = MI_weight
        self.MI_regu = MI_regu
        self.MI_batchSize = MI_batchSize
        ...
        self.MI_loss = self.estimator_quantities['mi_for_grads']

        if self.MI_regu:
            self.regressorLoss = (loss0+reg)/self.n_class
            self.weighted_MI_Loss =  self.MI_weight * self.MI_loss
            self.reconstruction_loss = self.regressorLoss + self.weighted_MI_Loss
        else:
    ...
    def train(self, epoches=1.0, batchSize = 200, minimalStep = 100):
        ...
        if self.MI_regu:
            self.batchSize = batchSize = self.MI_batchSize
        ...
        for i in range(step):
                ...
                if i==0:
                    self.sess.run(tf.global_variables_initializer(), feed_dict={self.x:batch,self.y:y_batch})

                if self.MI_regu:
                    _, self.loss, self.R2, self.MI, self.projMat = self.sess.run([self.optimizer,self.reconstruction_loss, self._R2, self.estimator_quantities["mi_for_grads"], self.U],feed_dict={self.x:batch,self.y:y_batch})
                    self.sess.run(self.estimator_train_op, feed_dict={self.x:batch, self.y:y_batch})
                else:
                ...
                if self.printOutput:
                    if self.MI_regu:
                        # print('step {:d} regressor loss: {:.2f} weighted MI loss: {:.2f}'.format(i, float(self.regressorLoss), float(self.weighted_MI_Loss)))
